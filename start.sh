...
print_bold_with_outline "Welcome to Duck V4, user!"
...
echo -e "${BLUE}[1/4] Cloning Duck Unblocker repository...${RESET}"
...
echo -e "${BLUE}[3/4] Starting Duck Unblocker...${RESET}"
npm start

if [ $? -eq 0 ]; then
    echo -e "${GREEN}[Success] Duck Unblocker started successfully.${RESET}"
    echo -e "${YELLOW}[Info] You can access Duck V4 via localhost.${RESET}"
else
    echo -e "${RED}[Error] Failed to start Duck Unblocker. Please check the application logs for details.${RESET}"
    exit 1
fi
